...
//we use the repr(transparent) attribute.
struct ColorCode(u8);
...
struct Buffer {
    //This tells(volatile) the compiler that the write has side effects and should not be optimized away.
    chars: [[Volatile<ScreenChar>; BUFFER_WIDTH]; BUFFER_HEIGHT], // 25 X 80
}
